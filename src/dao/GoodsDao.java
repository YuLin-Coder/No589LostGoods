...
public class GoodsDao {
	//获取连接执行查询
		...
		public boolean insertGoods(Goods g) {
			try {
				//执行插入sql
				runner.update("insert into goods(gname,lostPlace,u_id,info,createTime,status,phone,imgUrl)values(?,?,?,?,?,?,?,?)",
						g.getGname(),g.getLostPlace(),g.getU_id(),g.getInfo(),g.getCreateTime(),g.getStatus(),g.getPhone(),g.getImgUrl());
		...
		//根据失主查询所有失物信息
		public List<Goods> findMyInfo(int u_id) {
			try {
				return runner.query("select * from goods where u_id=?", new BeanListHandler<Goods>(Goods.class),u_id);//添加实体类的适配器进行类的反射
			} catch (SQLException e) {//捕获异常
				throw new RuntimeException(e);//抛出运行异常
			}
		...
		public boolean updateGoods(Goods g,int id) {
			try {//执行更改
				runner.update("update goods set gname=?,lostPlace=?,info=?,createTime=?,status=?,phone=? where id=?",
						g.getGname(),g.getLostPlace(),g.getInfo(),g.getCreateTime(),g.getStatus(),g.getPhone(),id);
			} catch (SQLException e) {
				throw new RuntimeException(e);//抛出运行异常
			}
